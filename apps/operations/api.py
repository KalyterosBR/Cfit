...
class PhysicalAssessmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        academy, unit = get_request_scope(self.request.user)
        queryset = PhysicalAssessment.objects.select_related("student", "evaluator").filter(student__academy=academy)
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(unit=unit, evaluator=self.request.user)
...
class GroupClassViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        academy, unit = get_request_scope(self.request.user)
        queryset = GroupClass.objects.prefetch_related("bookings__student").filter(academy=academy)
        return queryset.filter(unit=unit) if unit else queryset
    def perform_create(self, serializer):
        academy, unit = get_request_scope(self.request.user)
        if not unit:
        ...
        serializer.save(academy=academy, unit=unit, instructor=serializer.validated_data.get("instructor", self.request.user))
    @action(detail=True, methods=["post"])
    def book(self, request, pk=None):
        group_class = self.get_object(); student = Student.objects.filter(pk=request.data.get("student"), academy=group_class.academy, unit=group_class.unit).first()
    ...
    def booking(self, request, pk=None, booking_id=None):
        group_class = self.get_object(); booking = group_class.bookings.get(pk=booking_id); serializer = ClassBookingSerializer(booking, data=request.data, partial=True); serializer.is_valid(raise_exception=True); saved = serializer.save()
    ...
    def cancel(self, request, pk=None):
        ...
        group_class.canceled = True; group_class.save(update_fields=["canceled", "updated_at"])
    ...
    def duplicate(self, request, pk=None):
        ...
        serializer = self.get_serializer(data={"title": group_class.title, "modality": group_class.modality, "starts_at": starts_at, "ends_at": ends_at, "capacity": group_class.capacity, "location": group_class.location})
        serializer.is_valid(raise_exception=True)
        copy = serializer.save(academy=group_class.academy, unit=group_class.unit, instructor=group_class.instructor)
