...
export default function HomeSystem() {
    ...
    function renderPanel(index: number) {
        ...
        return <div className="grid h-full gap-5 lg:grid-cols-[0.25fr_0.75fr] lg:items-center">
            <div><span className="flex h-10 w-10 items-center justify-center rounded-full border border-blue-300/25 bg-blue-400/10 text-cyan-300"><Icon size={18} /></span><p className="mt-4 text-[9px] font-black uppercase tracking-[0.22em] text-cyan-300">{slide.eyebrow}</p><h3 className="mt-2.5 max-w-md text-3xl font-black leading-[1.02] tracking-[-0.05em] text-white sm:text-[2.1rem]">{slide.title}</h3><p className="mt-3 max-w-md text-sm leading-6 text-slate-300">{slide.description}</p><div className="mt-4 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400"><CheckCircle2 size={14} className="text-emerald-400" />Representação demonstrativa</div></div>
