...
const segmentOptions: Array<{
  ...
  { value: "plan_ending", label: "Plano próximo do fim" },
  { value: "without_workout", label: "Sem treino" },
  { value: "without_assessment", label: "Sem avaliação" },
  { value: "birthdays", label: "Aniversariantes do mês" },
  { value: "access_blocked", label: "Acesso bloqueado" },
  { value: "incomplete_profile", label: "Cadastro incompleto" },
];
...
export default function StudentsListPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() =>
    JSON.parse(
      localStorage.getItem("cfit_student_columns") ??
        '["plan","financial","health","due","frequency","status"]',
    ),
  );
  ...
  function toggleColumn(column: string) {
    setVisibleColumns((current) => {
      const next = current.includes(column)
        ...
        : [...current, column];
      localStorage.setItem("cfit_student_columns", JSON.stringify(next));
      return next;
    });
  }
  ...
  async function confirmToggleActive() {
    ...
    try {
      ...
      if (studentToToggle.active) {
        await deactivateStudent(studentToToggle.id, deactivationReason.trim());
      } else {
  ...
  return (
    <DashboardLayout>
      ...
      <ConfirmDialog
        open={openConfirm}
        title={isDeactivating ? "Inativar aluno" : "Reativar aluno"}
        description={
          isDeactivating
        ...
        confirmDisabled={isDeactivating && !deactivationReason.trim()}
