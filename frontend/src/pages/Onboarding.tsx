...
import { isValidEmail, normalizeEmail, phoneMask } from "@/utils/masks";
...
export default function Onboarding() {
    ...
    function next() {
        if (step === 1 && (!form.name.trim() || !form.trade_name.trim() || !form.establishment_type || ![10, 11].includes(form.phone.replace(/\D/g, "").length) || !isValidEmail(form.email))) return toast.error("Preencha telefone e e-mail válidos nesta etapa.");
        if (step === 2 && (!form.unit_name.trim() || form.unit_address.trim().length < 8 || ![10, 11].includes(form.unit_phone.replace(/\D/g, "").length) || !form.size_range)) return toast.error("Preencha todos os campos obrigatórios da unidade.");
