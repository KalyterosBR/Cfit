...
export function ForgotPassword() {
    ...
    return <Shell title="Recuperar senha" subtitle="Enviaremos um link se o e-mail estiver cadastrado.">{sent ? <div className="mt-6 rounded-xl bg-emerald-500/10 p-4 text-sm text-emerald-300">Solicitação recebida. Verifique o e-mail e a caixa de spam.</div> : <form onSubmit={submit} className="mt-6 space-y-4"><input required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="seu@email.com" className={field}/><button className="h-11 w-full rounded-xl bg-blue-600 font-bold">Enviar instruções</button></form>}<Link to="/" className="mt-5 block text-center text-sm text-cyan-300">Voltar ao login</Link></Shell>;
}

export function ResetPassword() {
    const [params] = useSearchParams(); const navigate = useNavigate(); const [password, setPassword] = useState("");
    async function submit(e: React.FormEvent) { e.preventDefault(); try { await Api.post("/users/password/reset/confirm/", { uid: params.get("uid"), token: params.get("token"), new_password: password }); toast.success("Senha redefinida."); navigate("/"); } catch { toast.error("Link inválido ou expirado."); } }
...
export function ChangePassword() {
    ...
    async function submit(e: React.FormEvent) { e.preventDefault(); try { await Api.post("/users/password/change/", { current_password: current, new_password: next }); clearTokens(); toast.success("Senha alterada. Entre novamente."); navigate("/", { replace: true }); } catch { toast.error("Confira a senha atual e os requisitos da nova senha."); } }
