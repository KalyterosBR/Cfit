...
4. **Refinar carregamento dos relatórios** — visões salvas já são persistidas; falta carregamento independente por bloco e invalidação de cache orientada por eventos.
5. **Personalização avançada do Dashboard** — reordenação, indicadores favoritos e padrões administrativos por perfil/unidade.
6. **Evoluir documentos** — versionamento, aceite imutável, renovação, arquivamento e download autorizado já existem; falta armazenamento privado externo, modelos reutilizáveis e política de retenção.
...
- evolução de carga auditada, funil paginado com contatos/propostas e segmentos reutilizáveis em sandbox;
- documentos com renovação por versão, arquivamento e aceite imutável;
- relatórios com cache curto por escopo e indicadores persistidos de agenda, turmas, comercial e treinos;
- comparação de unidades com ranking, alertas e referência ao período anterior;
